...
function($, Backbone, _, Handlebars, Modernizr, Toucheffects, OtherTpl) {
  return Backbone.View.extend({

    el: '#other',
    template: Handlebars.compile(OtherTpl),
    events: {
    ...
    initialize: function() {
      ...
      $.get( "/get_other_images_count", function(data) {
    ...
    preload_images: function() {
      var self = this;
      if($('body').data('active_view') == 'other' && $('body').data('preload')) {
        if(this.preload_index <= this.images_count/2) {
          require(
          [
            'image!app/img/other/' + self.preload_index + '.jpg'
          ],
          function() {
            self.preload_index++;
            self.preload_images();
    ...
    load_more_images: function(e) {
      ...
      for (var i = this.image_index; i < images_count + this.image_index; i++) {
        self.images.push('image!app/img/other/' + i + '.min.jpg');
      }
    ...
    get_right_slide: function(e) {
      ...
      img.fadeOut(420, function() {
        if(self.slider_index < (self.images_count - 1)) {
          self.slider_index++;
          var img_url = 'app/img/other/' + self.slider_index + '.jpg';
          requirejs(
          [
            'image!' + img_url,
          ],
          ...
          img.attr('src', 'app/img/other/' + (self.images_count - 1) + '.jpg')
            .show();
          $(e.target).hide();
          $('#gallery_preloader').fadeOut(110);
        }
    ...
    get_left_slide: function(e) {
      ...
      img.fadeOut(420, function() {
        if(self.slider_index > 1) {
          ...
          img.attr('src', 'app/img/other/' + self.slider_index + '.jpg');
          img.fadeIn(420);
          $('#gallery #right_img').show();
        } else {
          img.attr('src', 'app/img/other/' + 1 + '.jpg')
            .show();
          $(e.target).hide();
          $('#gallery_preloader').fadeOut(110);
        }
