...
function($, Backbone, _, Handlebars, Modernizr, Toucheffects, Photo_shootTpl) {
  return Backbone.View.extend({

    el: '#photo_shoot',
    template: Handlebars.compile(Photo_shootTpl),
    events: {
    ...
    initialize: function() {
      ...
      $.get( "/get_photo_shoot_images_count", function(data) {
    ...
    preload_images: function() {
      var self = this;
      if($('body').data('active_view') == 'photo_shoot' && $('body').data('preload')) {
        if(this.preload_index <= this.images_count/2) {
          require(
          [
            'image!app/img/photo_shoot/' + self.preload_index + '.jpg'
          ],
          function() {
            self.preload_index++;
            self.preload_images();
    ...
    load_more_images: function(e) {
      ...
      for (var i = this.image_index; i < images_count + this.image_index; i++) {
        self.images.push('image!app/img/photo_shoot/' + i + '.min.jpg');
      }
    ...
    get_right_slide: function(e) {
      ...
      img.fadeOut(420, function() {
        if(self.slider_index < (self.images_count - 1)) {
          self.slider_index++;
          var img_url = 'app/img/photo_shoot/' + self.slider_index + '.jpg';
          requirejs(
          [
            'image!' + img_url,
          ],
          ...
          img.attr('src', 'app/img/photo_shoot/' + (self.images_count - 1) + '.jpg')
            .show();
          $(e.target).hide();
          $('#gallery_preloader').fadeOut(110);
        }
    ...
    get_left_slide: function(e) {
      ...
      img.fadeOut(420, function() {
        if(self.slider_index > 1) {
          ...
          img.attr('src', 'app/img/photo_shoot/' + self.slider_index + '.jpg');
          img.fadeIn(420);
          $('#gallery #right_img').show();
        } else {
          img.attr('src', 'app/img/photo_shoot/' + 1 + '.jpg')
            .show();
          $(e.target).hide();
          $('#gallery_preloader').fadeOut(110);
        }
