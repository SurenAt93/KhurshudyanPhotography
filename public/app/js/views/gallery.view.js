...
[
  ...
  'modernizr',
  'toucheffects',
  'require.text!tpl/gallery.tpl',
],
function($, Backbone, _, Handlebars, Modernizr, Toucheffects, GalleryTpl) {
  return Backbone.View.extend({

    el: '#gallery_o',
    template: Handlebars.compile(GalleryTpl),
    events: {
    ...
    initialize: function() {
      ...
      requirejs(
      [
        'image!app/img/gallery/main.jpg'
      ],
      function() {
        self.$el.css('background-image',
          'url("app/img/gallery/main.jpg")');
      });
      $.get( "/get_gallery_images_count", function(data) {
    ...
    preload_images: function() {
      var self = this;
      if($('body').data('active_view') == 'gallery' && $('body').data('preload')) {
        if(this.preload_index <= this.images_count/2) {
          require(
          [
            'image!app/img/gallery/' + self.preload_index + '.jpg'
          ],
          function() {
            self.preload_index++;
            self.preload_images();
    ...
    load_more_images: function(e) {
      ...
      for (var i = this.image_index; i < images_count + this.image_index; i++) {
        self.images.push('image!app/img/gallery/' + i + '.min.jpg');
      }
    ...
    get_right_slide: function(e) {
      ...
      img.fadeOut(420, function() {
        if(self.slider_index < (self.images_count - 1)) {
          self.slider_index++;
          var img_url = 'app/img/gallery/' + self.slider_index + '.jpg';
          requirejs(
          [
            'image!' + img_url,
          ],
          ...
          img.attr('src', 'app/img/gallery/' + (self.images_count - 1) + '.jpg')
            .show();
          $(e.target).hide();
          $('#gallery_preloader').fadeOut(110);
        }
    ...
    get_left_slide: function(e) {
      ...
      img.fadeOut(420, function() {
        if(self.slider_index > 1) {
          ...
          img.attr('src', 'app/img/gallery/' + self.slider_index + '.jpg');
          img.fadeIn(420);
          $('#gallery #right_img').show();
        } else {
          img.attr('src', 'app/img/gallery/' + 1 + '.jpg')
            .show();
          $(e.target).hide();
          $('#gallery_preloader').fadeOut(110);
        }
