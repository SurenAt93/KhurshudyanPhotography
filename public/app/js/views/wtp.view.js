...
[
  ...
  'modernizr',
  'toucheffects',
  'require.text!tpl/wtp.tpl',
],
function($, Backbone, _, Handlebars, Modernizr, Toucheffects, WtpTpl) {
  return Backbone.View.extend({

    el: '#when_talking_pictures',
    template: Handlebars.compile(WtpTpl),
    events: {
    ...
    initialize: function() {
      ...
      requirejs(
      [
        'image!app/img/wtp/main.jpg'
      ],
      function() {
        self.$el.css('background-image',
          'url("app/img/wtp/main.jpg")');
      });
      $.get( "/get_wtp_images_count", function(data) {
    ...
    load_more_images: function(e) {
      ...
      for (var i = this.image_index; i < images_count + this.image_index; i++) {
        self.images.push('image!app/img/wtp/' + i + '.jpg');
      }
    ...
    get_right_slide: function(e) {
      ...
      img.fadeOut(420, function() {
        if(self.slider_index < (self.images_count - 1)) {
          self.slider_index++;
          var img_url = 'app/img/wtp/' + self.slider_index + '.jpg';
          requirejs(
          [
            'image!' + img_url,
          ],
    ...
    get_left_slide: function(e) {
      ...
      img.fadeOut(420, function() {
        if(self.slider_index > 1) {
          ...
          img.attr('src', 'app/img/wtp/' + self.slider_index + '.jpg');
          img.fadeIn(420);
